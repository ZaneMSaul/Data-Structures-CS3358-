...
namespace CS3358_ssII2018_A7
{
	...
	p_queue::p_queue(size_type initial_capacity) : capacity(initial_capacity > 0 ? initial_capacity : DEFAULT_CAPACITY), used(0)
	{
	...
	p_queue& p_queue::operator=(const p_queue& rhs)
	{
		if (this != &rhs) {
			...
			for (size_type i = 0; i < rhs.capacity; i++) {
	...
	void p_queue::push(const value_type& entry, size_type priority)
	{
		...
		ItemType newHeap;
		newHeap.data = entry;
		newHeap.priority = priority;
		heap[used] = newHeap;
		used++;
		if (used != 1)
			for (size_type i = used - 1; i != 0 && parent_priority(i) < heap[i].priority; i = parent_index(i))
				swap_with_parent(i);
	}
	...
	void p_queue::pop()
	{
		if (used > 1) 
		{
			...
			used--;
			for (size_type index = 0, bigChild = index;
				!is_leaf(index) 
				&& big_child_priority(index) > heap[index].priority
				&& big_child_index(index) < used; )
			{
				bigChild = big_child_index(index);
	...
	{
		assert(!is_leaf(i));
		if (heap[i * 2 + 1].priority < heap[i * 2 + 2].priority && (i * 2 + 2) < used)
			return (i * 2 + 2);
		else
			return (i * 2 + 1);
	}
