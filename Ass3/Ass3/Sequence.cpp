...
{
    if (used == capacity)
        resize(capacity * 1.25 == capacity ? capacity + 1 : capacity * 1.25);
    if (used == current_index)
        current_index = 0;
    for (size_type i = used, k = used - 1; i > current_index; i--, k--)
        data[i] = data[k];
...
{
    if (used == capacity)
        resize(capacity * 1.25 == capacity ? capacity++ : capacity * 1.25);
    ...
    {
        for (size_type i = used, k = used - 1; i > current_index + 1;
             i--, k--)
            data[i] = data[k];
...
{
    assert(is_item());
    for (size_type i = current_index, k = i + 1; i < used - 1; i++, k++)
        data[i] = data[k];
    used--;
}
