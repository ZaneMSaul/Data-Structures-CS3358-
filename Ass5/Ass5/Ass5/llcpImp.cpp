...
void   InterleaveOddsAndEvensInOrigOrder(Node*& headPtr)
{
    ...
    if( headPtr != nullptr && headPtr->link != nullptr &&
        headPtr->link->link != nullptr )
    for( Node *curssor = headPtr, *tempHolding = headPtr->link;
        tempHolding != nullptr; NULL)
    {
        curssor = headPtr;
        tempHolding = headPtr->link;
        for( NULL; tempHolding != nullptr; curssor = curssor->link,
            tempHolding = curssor->link)
            if( curssor->link->link != nullptr )
                if( curssor->data % 2 == curssor->link->data % 2 &&
                    curssor->data % 2 != curssor->link->link->data % 2 )
                {
