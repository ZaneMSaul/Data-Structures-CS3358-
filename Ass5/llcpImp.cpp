...
void   InterleaveOddsAndEvensInOrigOrder(Node*& headPtr){
    if( headPtr != nullptr && headPtr->link != nullptr && headPtr->link->link != nullptr ){
        bool needToQuit = false;
        while( !needToQuit ){
            Node* curssor = headPtr;
            Node* tempHolding = headPtr->link;
            while( curssor != nullptr ){
                if( curssor->link != nullptr && curssor->link->link != nullptr )
                    if( ( curssor->data % 2 == curssor->link->data % 2 ) && ( curssor->data % 2 != curssor->link->link->data % 2)) {
                        curssor->link = curssor->link->link;
                        tempHolding->link = curssor->link->link;
                        curssor->link->link = tempHolding;
                        break;
                    }
                curssor = curssor->link;
                tempHolding = curssor->link;
                if( tempHolding == nullptr )
                {
                    needToQuit = true;
                    break;
                }
  ...
  /*  
    if( headPtr != 0 || headPtr->link != 0 || headPtr->link->link !=0 )
        return;
    ...
    while( !needToQuit )
    {
        ...
        while( cursor->link != nullptr && cursor->link->link != nullptr )
        {
            preTracer = cursor;
            cursor = preTracer->link;
            forTracer = cursor->link;
            
            if( ( preTracer->data % 2 == cursor->data % 2 ) && ( cursor->data % 2 != forTracer->data % 2) )
            {
                Node* temp = cursor;

                preTracer->link = forTracer;
                cursor->link = forTracer->link;
                forTracer->link = temp;
                break;
            }
            if( forTracer->link == 0 )
