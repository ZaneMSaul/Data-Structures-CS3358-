...
{
	size_type homeLoc = hash(cStr), newLoc = homeLoc, i = 1;

	while (i < capacity)
	{
		if (!strcmp(data[newLoc].word, cStr))
		{
			return true;
		}
		newLoc = ((homeLoc + (i * i)) % capacity);
		++i;
	}
	return false;
}
...
{
	size_type homeLoc = hash(cStr), newLoc = homeLoc;
	for (size_type i = 1; *(data[newLoc].word) != '\0'; i++)
	{
		newLoc = ((homeLoc + (i * i)) % capacity);
	}
	strcpy(data[newLoc].word, cStr);
	used++;
	if (load_factor() > 0.45)
	{
		rehash();
